...
<script>
    $(function() {
        ...
        $("#gFile").on("change", function(e) {
            const galleryContainer = $("#galUpload");
            const galleryFiles = this.files;

            // Clear existing previews
            galleryContainer.find(".gitems").remove();

            // Append new previews
            $.each(galleryFiles, function(index, file) {
                const imageElement = `
                <div class="item gitems">
                    <img src="${URL.createObjectURL(file)}" alt="Gallery Image Preview" />
                </div>`;
                galleryContainer.prepend(imageElement);
            });
    ...
    function stringToSlug(text) {
        return text.toLowerCase()
            .replace(/[^\w\s-]/g, "") // Remove anything that's not a word character, space, or hyphen
